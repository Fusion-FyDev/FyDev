...
class ModulePa(ModuleRepository):
    '''Module wrapper.
    ...
    def _execute_modulecmd(self, cmd, *args):
        ...
        # if completed.stderr == '':
        #     return namespace.get('_mlstatus')
        # else :
    ...
    def listpa(self, *args,max_list ,**kwargs):
        # to list the related modulename which is installed 

        fullname = self.initpa(*args,**kwargs)
        if max_list == True:
            fullname = self.name
    ...
    def checkpa(self,*args,**kwargs):
        ...
        if fullname.find("/") != -1:
            try:
                ...
                # self.execute('purge')
                #we need to run puege and then module load 
                output = self.execute('load',fullname)
                logger.debug(f"the output is {output}")
                return output
            except output == 'None'   :
                logger.debug(f"the pacakes {fullname} is not install in this system , \
                and I list all other version in this systerm ; \
    ...
    def listdepend(self,*args,**kwargs):
        logger.debug("the install is finish ,you can use it by module load modulename \
                if the checkpa(new package) is fail ,to prove the install is fall ")  
        fullname = self.initpa(*args,**kwargs)
        path=self.path
        listdependflag = 0
        dependlist=[]
        if self.checkpa(fullname) == False:
            self.load_configure(path)
            # if self.checkpa(fullname) == False:
            environ = 'GCC'
    ...
    def fetchsources(self,*args,**kwargs):
        ...
        if self.checkpa(fullname) == False:
            ...
            fetch_command=["the software is installed befor"]
